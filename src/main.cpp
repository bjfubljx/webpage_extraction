...
{
    string s;
    if(IsBeginWith(c.get_release_time(),"("))
    {
        s = c.get_release_time().substr(1, c.get_release_time().find(" "));
        cout<<"s: "<<s<<endl;
    }
    else
    {
        s = c.get_release_time().substr(0, c.get_release_time().find(" "));
    }
    ...
    cout<<"forth"<<endl;
    out.close();
    chdir("/home/fengfan/Documents/pc/");
    cout<<"fifth"<<endl;
}
void setobject()
{
...
int main(int argc, char const *argv[])
{
    ...
    cout<<"got:"<<got<<endl;
    int res;
    res = bf.VaryExist(got.c_str());
	if(res)
		cout << "exist" << endl;
	else
		cout << "not exist" << endl;
    res = bf.VaryExist(a.c_str());
	if(res)
		cout << "exist" << endl;
	else
		cout << "not exist" << endl;
        */
    worklist url_sets;
    url_sets.pushone("http://politics.gmw.cn/2019-01/14/content_32344640.htm");
    BloomFilter bf(2000, 100);
    int num=0;
    while (true)
    {
        ...
        Request req(url);
        Response res;
        req.setHeader(req.getPath(), req.getHost());
        res = req.getPageContent(req.getHost(), req.getHeader());
        for (int i = 0; i < res.getbody().length(); i++)
        {
            buf[i] = res.getbody().at(i);
        }
        ...
        auto sp_urls = GetGmwPolic(res.getbody());

        for (auto sp_url : sp_urls)
        {
            cout<<"all:"<<sp_url;
            //条件判断 放入 url_sets
            if (bf.VaryExist(sp_url.c_str()) == 0)
            {
                cout<<"right:"<<sp_url<<endl;
                bf.SetKey(sp_url.c_str());
                url_sets.pushone(sp_url);
            }
        }
    }
    return 0;
