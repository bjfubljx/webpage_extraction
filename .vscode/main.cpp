...
int main(int argc, char const *argv[])
{
	...
	if(res)
        ...
        */
    worklist url_sets;
    url_sets.pushone("http://blog.sina.com.cn/s/blog_4e780db10102ykyy.html?tj=2");
    BloomFilter bf(2000, 100);
    ...
    Request req(url);
    Response res;
    string raw_body=res.getbody();
    string raw_url=url;
    req.setHeader(req.getPath(),req.getHost());
    res=req.getPageContent(req.getHost(),req.getHeader());
    for(int i = 0;i < raw_body.length();i++)
    {
        buf[i] = raw_body.at(i);
    }
    for(int i = 0;i < raw_url.length();i++)
    {
        ur[i] = raw_url.at(i);
    }
    setobject();
    auto sp_urls = GetSinaBlog(res.getbody());
